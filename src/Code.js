/*************************************************************************************************
 * Naver News Fetching Bot (v2.2.3)
 * ***********************************************************************************************
...
async function postArticle(g, pubDateText, title, source, description, link) {
  ...
  if (g.allowBotSlack) {
    const params = {
      ...
      payload: JSON.stringify(
        createArticleCardSlack(pubDateText, title, source, description, link),
      ),
    };
  ...
  if (g.allowBotTeams) {
    const params = {
      ...
      contentType: "application/json",
      payload: JSON.stringify(
        createArticleCardTeams(pubDateText, title, source, description, link),
      ),
    };
  ...
  if (g.allowBotJandi) {
    const params = {
      ...
      payload: JSON.stringify(
        createArticleCardJandi(pubDateText, title, source, description, link),
      ),
    };
  ...
  if (g.allowBotGoogle) {
    const params = {
      ...
      contentType: "application/json",
      payload: JSON.stringify(
        createArticleCardGoogle(pubDateText, title, source, description, link),
      ),
    };
...
function runFetchingBot() {
  ...
  if (
    ...
    Logger.log(
      "* 뉴스봇 및 아카이빙 기능이 모두 false로 설정되어 있습니다. 설정값들을 다시 확인해주세요.\n",
    );
    return;
  }
  ...
  const lastArticleUpdateTime = getProperty("lastArticleUpdateTime")
    ? new Date(parseFloat(getProperty("lastArticleUpdateTime")))
    : null;
  const registeredKeyword = getProperty("registeredKeyword")
    ? getProperty("registeredKeyword")
    : null;
  let feed;
  ...
  if (feed.getResponseCode() == 200) {
    ...
    if (!lastArticleUpdateTime) {
      ...
      if (registeredKeyword != g.keyword) {
        Logger.log(
          "* 네이버뉴스 검색 키워드 변경 : '" + registeredKeyword + "' -> '" + g.keyword + "'\n",
        );
